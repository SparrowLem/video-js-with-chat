	...
	class ChatContainer extends videojs.getComponent('Component') {
		...
		constructor(player, options) {
			...
			let controlChat = player.getChild('ControlBar').addChild('button', {
				...
				let icon = document.querySelector('.chat-icon-placeholder')
				icon.classList.toggle('chat-icon-placeholder-close');
				}				
              });
			...
			iconControlChat.classList.add('chat-icon-placeholder');
			...
			let inssertMessages = function(array){
				for (let i = 0; i<array.length; i++) {
					...
					if (array[i].isPinned){
						...
						chatMessages.classList.remove('chat-usermes-full');
						chatMessages.classList.add('chat-usermes-w-pin');
